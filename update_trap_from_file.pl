...
my $USAGE = "update_trap_from_file.pl [-p project_id] [-c check] [-f file] [-h help]";
my ($p_id, $check, $file, $help);

&GetOptions(    	'project_id|p=s'		=> \$p_id,
			'file|f=s'			=> \$file,
			'check|c'			=> \$check,
			'help|h'           		=> \$help);
...
if (!$p_id) {
	print STDOUT "You must specify a project.\n";
	exit;
}

if (!$file) {
	print STDOUT "You must specify a file to parse.\n";
	exit;
}

if ($help) {
	print STDOUT "HELP: nohup  perl update_trap_from_file.pl -c yes -p 4 -file ./ggtc.gb > /tmp/roma/update_trap.out &\n";
	exit;
}
...
my $seqio = Bio::SeqIO->new(-format => 'genbank', -file => $file);
my $sql_projects = "select project_name from project where project_name != 'tigem' and project_id = $p_id order by project_id;";
...
while (my $rhref_projects = $sth_projects->fetchrow_hashref) {
	...
	while (my $seq = $seqio->next_seq) {
		...
		if ($check == 1) {
			my $sql_check = "select distinct trap_id from trap where trap_name = '$trap_name' and project_id = $p_id;";
