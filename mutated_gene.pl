...
$id = $ARGV[0];
$id = $id - 1; # in order to make the limit
$idcount = $ARGV[1];
...
my $sth = $trapdb->prepare("select distinct ensembl_id, gene_name, chr, gene_start, gene_end, gene_strand, gene_description, gene_type, refseq_id, count(unitrap_id) as c from unitrap group by ensembl_id limit $id,$idcount;");
$debug && print STDOUT "SQL CODE: select distinct ensembl_id, gene_name, chr, gene_start, gene_end, gene_strand, gene_description, gene_type, refseq_id, count(unitrap_id) as c from unitrap group by ensembl_id limit $id,$idcount;\n";
$sth->execute() || die "insert failed : $DBI::errstr";

while (my $hit_ids = $sth->fetchrow_hashref) {
	my $ensembl_id = $hit_ids->{'ensembl_id'};
	my $chr = $hit_ids->{'chr'};
	...
	my $ensdb = new Bio::EnsEMBL::DBSQL::DBAdaptor(	-host => $enshost,
							-user => $ensuser,
							-pass => $enspass,
							-dbname => $db)|| die "Can't connect: ", $DBI::errst;
